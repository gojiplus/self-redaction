...
The record matcher uses field-specific rules rather than applying fuzzy search to every value. It normalizes common phone, date, address, order, and account formats. For full names at least seven characters long, it accepts a Levenshtein distance of one. It also permits one edit in a street name when the house number and suffix agree exactly. The implementation uses [RapidFuzz 3.14.5](https://rapidfuzz.github.io/RapidFuzz/Usage/distance/Levenshtein.html), pinned in the lock file.
